...
country,
price,
rank() over(partition by country order by price desc) as rnk
...
c.Customer_id,
c.Customer_name 
from transaction t join customers c on t.Customer_id=c.Customer_id 
where  Year(Transaction_Date)=EXTRACT(YEAR FROM current_date) 
